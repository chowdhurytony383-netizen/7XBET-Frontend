...
function getMatchKey(match) {
  return match?._id || match?.id || `${match?.league || ''}-${match?.homeTeam?.name || match?.home || ''}-${match?.awayTeam?.name || match?.away || ''}`;
}
...
function normalizeOdds(match) {
  ...
  if (markets && typeof markets === 'object') {
    return Object.entries(markets).slice(0, 9).map(([label, value]) => ({ key: label, label, value }));
  }
  return [];
}
    ...
    <article className="live-match-row">
      <div className="live-match-info">
        <div className="live-league-line">
          ...
          <strong>{league || 'Football'}</strong>
        </div>
        <div className="live-teams">
          <span>{teamName(homeTeam)}</span>
          <span>{teamName(awayTeam)}</span>
        </div>
      </div>
      <div className="live-score-box">
        <span className="live-pill">{status}</span>
        <div className="live-score-values">
          <span>{scoreValue(match, 'home')}</span>
          <span>{scoreValue(match, 'away')}</span>
        </div>
      </div>
    ...
    <section className="live-sports-section" id="live">
      ...
      <div className="live-sports-grid">
        <div className="live-match-list">
          {matches.length ? (
            matches.slice(0, 5).map((match) => <LiveMatchRow key={getMatchKey(match)} match={match} />)
          ) : (
            <EmptyState title="Live matches unavailable" message="Add live sports data in the backend to show this section." />
          )}
        </div>
